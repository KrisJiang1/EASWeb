...
export default {
  ...
  methods: {
    ...
    //通过专业查询该专业的班级 (当专业change时调用)
    selectClazzByMajor(){
      this.$http.post(
        "api/eas/major/selectMajorByCondition",
        this.student.studentMajor
      ).then(
        res=>{
          console.log(res.data.data);
          this.formMsg.allClazzs = res.data.data[0].clazzList;
          this.student.studentClazz.clazzId=this.formMsg.allClazzs[0].clazzId;
        }
      )
    }
  },
            ...
            res=>{
              ...
              this.$http.post("api/eas/student/selectStudentByCondition",
                ...
                res=>{
                  ...
                  this.student.studentDormitory = [res.data.data[0].studentDormitory.dormitoryBuilding.dormiBuilId,res.data.data[0].studentDormitory.dormitoryId];
