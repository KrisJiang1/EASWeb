<template>
  <div class = "updateClazz">
    <el-form :model="clazz" ref="clazz" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item>
        <el-button type="primary" @click="updateClazz">立即修改</el-button>
...
export default {
  ...
  methods:{
    ...
    updateClazz(){
      this.$http.put("api/eas/clazz/updateClazz",
        this.clazz).then(
        res=>{
          let body = res.data;
          if(body.status == '100'){
  ...
  created() {
    let clazzId = this.$route.query.i;
    this.$http.get("api/eas/major/selectAllMajor").then(
      res=> {
        ...
        this.$http.post("api/eas/post/selectPostByCondition",
          ...
          //通过clazzId对班级信息进行数据回显
          this.$http.post("api/eas/clazz/selectClazzByCondition",{clazzId:clazzId}).
          then(res=>{
            this.clazz.clazzId = clazzId;
            let clazz = res.data.data[0];
            this.clazz.clazzName = clazz.clazzName;
            this.clazz.gradeName = clazz.gradeName;
            this.clazz.clazzMajor.majorId = clazz.clazzMajor.majorId;
            this.clazz.clazzInstructor.staffId = clazz.clazzInstructor.staffId;
          })
